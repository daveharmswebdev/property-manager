...
- **NEVER log emails, user/account IDs, storage keys, or request fields — even when wrapped in `LogSanitizer.MaskEmail`, `MaskId`, `MaskStorageKey`, or `Sanitize`.** Two CodeQL queries block merges on this: CWE-359 ("Exposure of private information") for PII, and `cs/cleartext-storage-of-sensitive-information` (HIGH) for account/tenant IDs. Their taint analysis does NOT recognize the custom masking helpers as safe sinks and will flag every PR that logs a wrapped value. Prefer structured correlation IDs (`InvitationId`, `UserId` of an admin actor, trace IDs) — they give the same diagnostic value without tripping the scanner. Stories 22-1 (`5e52256`), 22-2 (`f0a1da7`), and 22-3 (autofix `be41b7f`) all shipped sensitive-value logs that had to be ripped out post-PR; do not be the fourth. **Canonical pattern + decision rule: see `architecture.md` → Implementation Patterns → Logging Pattern → "CodeQL-safe logging".**
