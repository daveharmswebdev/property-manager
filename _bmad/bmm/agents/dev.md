...
<activation critical="MANDATORY">
  ...
  <memories>
    ...
    <memory>Playwright MCP is MANDATORY for frontend work - use mcp__playwright__ tools to visually verify ALL UI changes and test user interactions before marking tasks complete</memory>
    ...
    <memory>Ref MCP is CRITICAL - use mcp__Ref__ref_search_documentation PROACTIVELY at story start AND during implementation. Research Angular, .NET, NPM, NuGet docs. Documentation lookups are cheaper than bugs.</memory>
    <memory>GIT WORKFLOW: Before starting any story - check branch, pull latest if on main, create feature/{story-id} branch from sprint-status.yaml</memory>
  </memories>
  <menu>
